...
async function analyzeCollection({ projectId }) {
  ...
  for (let pct of baseConfig.analyze) {
    ...
    console.log('resultsTokenList.length', resultsTokenList.length);
  }
...
function createAnalyzeWebPage(tokenLists, tokenFinalList, config) {
  let html = createWebPageHead();

  console.log('tokenPartLists.length', tokenLists.length);
  console.log('tokenPartLists[0]', tokenLists[0]);
  ...
  html = html + `<tr>`;
  for (let i = 0; i < config.analyze[i]; i++) {
    html = html + `<td>${config.analyze[i]}</td>`;
  }
  html = html + `</tr>`;
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (let j = 0; i < tokenLists.length; i++) {
      const tokenId = tokenLists[j][i].tokenId;
      const token = getTokenFromList(tokenId, tokenFinalList);
      html = html + `<td>${token.rank}</td>`;
    }
    html = html + `</tr>`;
  }
  html = html + `</table></div>`;
  ...
  fs.writeFileSync(fileutil.toAbsoluteFilePath(`../config/projects/${config.projectId}/analyze.html`), html);
  opn(path, { app: 'chrome' });
}

function createWebPageHead(title = '') {
  let html = '';
...
function runOneAnalysis(config, tokenList) {
  ...
  debugToFile(config, 'analyzis1.json');
...
async function fetchCollectionMilestonesBak(milestones = [], config) {
  const timer = createTimer();
  while (true) {
    const numFinishedBefore = countDone(config);
    const nextTokens = getNextTokens(config, config.nextTokensBatchSize);
...
function calcGlobalRarity(config) {
  ...
  for (let trait of Object.keys(config.data.attributes)) {
    ...
    config.data.attributes[trait].numTraitValues = numTraitValues;
  }

  config.data.attributes.numTraitValues = numTraitValues;
  config.data.attributes.config.data.tokenURI = config.data.tokenURI;
  config.data.attributes.avgNumTraitValuesPerTraitType = numTraitValues / config.data.tokenURI;
  ...
  for (let trait of Object.keys(config.data.attributes)) {
    ...
    for (let value of Object.keys(config.data.attributes[trait].values)) {
      const rarityNormalized =
        ...
        (config.data.attributes.avgNumTraitValuesPerTraitType / config.data.attributes[trait].numTraitValues);
...
function createTimer() {
  ...
  return {
    ...
    ping: () => {
      ...
      log.info(`Duration: ${duration} secs`);
    },
    duration: () => {
      const duration = ((new Date()).getTime() - timer.lastDate.getTime()) / 1000;
      timer.lastDate = new Date();
