...
async function crawl(collection) {
  try {
    ...
    while (true) {
      ...
      collection.save();
      await utilslib.sleep(collection.config.sleepBetweenEachAssetBatch * 1000);
    }
  } catch (e) {
...
async function processAssets(collection, assetIds) {
  while (true) {
    ...
    for (const id of nextIds) {
      ...
      await utilslib.sleep(collection.config.sleepBetweenEachAsset);
    }

    const result = await Promise.all(requests);
    // todo: process result!
    collection.save();
    await utilslib.sleep(collection.config.sleepBetweenEachAssetBatch);
    // log.info(result);
  }
}
